...
var buildCakeFileName = MakeAbsolute(Directory(".")).FullPath + "/build.standard.cake";
var tempCakeBuildFileName = tempFolder + "/build.standard.cake.new";

var currentGitBranch = GitBranchCurrent(DirectoryPath.FromString("."));
var latestBuildCakeUrl = "https://raw.githubusercontent.com/aspenlaub/ShatilayaStandard/master/build.standard.cake?g=" + System.Guid.NewGuid();
...
Task("UpdateBuildCake")
  .Description("Update build.standard.cake")
  .Does(() => {
    ...
    if (Regex.Replace(oldContents, @"\s", "") != Regex.Replace(System.IO.File.ReadAllText(tempCakeBuildFileName), @"\s", "")) {
      ...
      throw new Exception("Your build.standard.cake file has been updated. Please check it in and then retry running it.");
    } else {
      System.IO.File.Delete(tempCakeBuildFileName);
    }
  });
...
Task("IgnoreOutdatedBuildCakePendingChangesAndDoCreateOrPushPackage")
  .Description("Default except check for outdated build.standard.cake, except check for pending changes and except nuget create and push")
...
Task("IgnoreOutdatedBuildCakePendingChangesAndDoNotPush")
  .Description("Default except check for outdated build.standard.cake, except check for pending changes and except nuget push")
...
Task("IgnoreOutdatedBuildCakePendingChanges")
  .Description("Default except check for outdated build.standard.cake and except check for pending changes")
...
Task("IgnoreOutdatedBuildCakeAndDoNotPush")
  .Description("Default except check for outdated build.standard.cake and except nuget push")
